...
class PageFilterSetting(Document):
    ...
    def get_category_names(self):
        # Get lft, rgt for the current category
        lft, rgt = frappe.db.get_value("Category", self.doctype_link, ["lft", "rgt"])
        
        # Fetch all enabled categories inside the current category (including itself)
        ...
        # If no children found (leaf node), just return itself
        if not categories:
            categories = [self.doctype_link]
