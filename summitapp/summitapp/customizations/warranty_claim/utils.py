...
def serial_number_details(kwargs):
    try:
        filed_names = get_field_names("Serial NO")
        if not kwargs.get('serial_no'): 
            return error_response('Please Specify Serial No')
...
def customer_warranty_claim_details(kwargs):
    try:
